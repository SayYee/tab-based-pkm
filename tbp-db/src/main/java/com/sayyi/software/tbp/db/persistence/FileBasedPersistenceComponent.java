 ...
 */
@Slf4j
public class FileBasedPersistenceComponent implements PersistenceComponent {
    ...
    public FileBasedPersistenceComponent(String snapDir) {
    ...
    private class RequestFileIterator implements Iterator<Request> {
        ...
        private void init(long lastOpId) throws IOException {
            ...
            Iterator<File> reversedIterator = Arrays.stream(files)
                    .sorted(Comparator.comparingLong(FileBasedPersistenceComponent.this::getFileOpId)
                                    .reversed())
                    .iterator();
